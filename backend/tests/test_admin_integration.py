...
def test_series_admin_reads_only_assigned_series_and_searches_active_users(db: Session) -> None:
...
def test_creating_a_series_adds_its_administrator_as_a_default_member(db: Session) -> None:
    suffix = uuid.uuid4().hex[:12]
    admin = User(
        oidc_issuer="https://issuer.test",
...
def test_series_admin_can_update_a_scheduled_round_and_its_status_is_recalculated(
    db: Session,
) -> None:
    suffix = uuid.uuid4().hex[:12]
    now = datetime.now(UTC)
...
def test_series_admin_can_replace_a_future_successor_plan_without_rewriting_rounds(
    db: Session,
) -> None:
    suffix = uuid.uuid4().hex[:12]
    now = datetime.now(UTC)
