...
class PlaylistManager:
    ...
    def finalize_month(
        ...
        *,
        ...
        with session_scope() as session:
            ...
            if playlist:
                logger.info("Playlist for %s already exists", month_start.strftime("%Y-%m"))
                return playlist
            ...
            if not submissions:
                logger.info("No submissions found for %s", month_start.strftime("%Y-%m"))
                return None
            ...
            logger.info(
                "Finalized playlist for %s with %d submissions",
    ...
    def sync_lastfm_stats_for_submission(self, submission_id: int) -> None:
        if self.lastfm_service is None:
            logger.debug("Last.fm service not configured; skipping stats sync for submission %d", submission_id)
            return
        ...
        with session_scope() as session:
            ...
            if not user.lastfm_username:
                logger.debug("User %s has no Last.fm username; skipping", user.username)
                return
            ...
            session.add(stat)
            logger.info("Updated Last.fm stats for submission %d", submission_id)