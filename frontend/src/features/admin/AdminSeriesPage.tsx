...
export function AdminSeriesPage() {
  ...
  const members = useQuery({
    queryKey: ["series-members", seriesId],
  ...
  const monthly =
    detail.data?.roundPlan?.kind === "calendar" && detail.data.roundPlan?.full_month === true;
  ...
  const createRound = useMutation({
    mutationFn: () =>
      post("/admin/rounds", {
        ...
        opens_at: zonedInputToIso(selectedOpensAt, seriesTimezone),
        closes_at: zonedInputToIso(selectedClosesAt, seriesTimezone),
        publish_at: zonedInputToIso(selectedPublishAt, seriesTimezone),
        submission_limit: Number(submissionLimit),
        contributor_user_ids: (members.data ?? []).map((member) => member.id),
    ...
    onSuccess: () => {
      ...
      setOpensAt("");
      setClosesAt("");
      setCloseEdited(false);
      setPublishAt("");
      setPublishEdited(false);
      setPrompt("");
      setPolicies([]);
      invalidate();
      showToast({
  ...
  function submitRound(event: FormEvent<HTMLFormElement>) {
    ...
    if (!openingIso || !closingIso || !publishingIso)
      return showToast({
        title: "Use valid series times",
        description:
          "One of these times does not exist in the series timezone, usually because of daylight saving time.",
        tone: "error",
      });
    if (new Date(selectedOpensAt) >= new Date(selectedClosesAt))
      return showToast({ title: "Closing must follow opening", tone: "error" });
    if (new Date(selectedClosesAt) > new Date(selectedPublishAt))
    ...
    <main className="shell admin-shell">
      ...
      <div className="admin-columns">
        ...
        <section className="panel" id="rounds">
          ...
          <form className="admin-round-form" noValidate onSubmit={submitRound}>
            <label>
              Title
              <input
                value={selectedTitle}
                ...
                onChange={(event) => {
                  setTitleEdited(true);
                  setRoundTitle(event.target.value);
                }}
              />
            </label>
            <label>
              ...
              <input
                ...
                onChange={(event) => {
                  ...
                  if (monthly && !closeEdited) setClosesAt("");
                  if (monthly && !publishEdited) setPublishAt("");
                  if (monthly && !titleEdited) setRoundTitle("");
                }}
              />
              <span className="field-hint">
                Times use {seriesTimezone}. Defaults to now, unless the latest round publishes
                later.
              </span>
            </label>
            <label>
            ...
            <label>
              ...
              <input
                type="datetime-local"
                min={selectedClosesAt || undefined}
                value={selectedPublishAt}
                required
                onChange={(event) => {
                  setPublishEdited(true);
            ...
            <p className="field-hint">
              {currentMembers.length
                ? `${currentMembers.length} current series member${currentMembers.length === 1 ? "" : "s"} will be included.`
            ...
            <button
              className="button"
              disabled={createRound.isPending || currentMembers.length === 0}
            >
